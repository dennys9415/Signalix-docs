...
| `V15__group_message_recipients.sql` | `group_message_recipients(message_id, recipient_user_id, recipient_device_id, ciphertext, encryption_version, pre_key_id, signed_pre_key_id, created_at)` with PK on the three id columns and index on `(recipient_user_id, message_id)` | v0.10.0 group E2EE beta. One row per (message × recipient × device). `messages.ciphertext` for a group encrypted send is an empty sentinel; the per-recipient ciphertext + envelope live here. `getMessages` LEFT JOINs this on `recipient_user_id = caller.id` to materialize the right body for each viewer. ON DELETE CASCADE from `messages` and `users` so soft / hard cleanups propagate. New joiners have no row for older messages and therefore can't decrypt them — intended property. |
